<template>
  <div class="data-table-container">
    ...
    >
      ...
      <!-- FragmentMethod Column -->
      <el-table-column
        prop="FragmentMethod"
        label="FragmentMethod"
        width="120"
        sortable="custom"
      />
      ...
      >
        <template #default="scope">
          <el-tooltip
            effect="light"
            :content="scope.row.PSM.toString()"
            :offset="-50"
            placement="right-start"
          >
            <el-progress
              :percentage="calculatePercentage(scope.row.PSM, maxPSMs)"
              :stroke-width="12"
              :show-text="false"
              class="data-progress"
            />
...
import {getMSNetDatasetDetails, getPrideDatasetDetails} from "@/api/getTable";
...
watch(
  ...
  (val) => {
    if (val && val.length > 0) {
      maxPSMs.value = Math.max(...val.map((o) => o.PSM));
  ...
  if (expandedRows.length > 0 && !datasetDetails.value[row.accession.id]) {
    ...
    try {
      ...
      if (row.accession.id.startsWith('PXD')) {
        ...
        const response = await getMSNetDatasetDetails(row.accession.path);
        details = {
          title: response.data.title,
